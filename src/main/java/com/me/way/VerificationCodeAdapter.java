...
//        out.write(imagedata);
        UploadImg.uploadVerImg(imagedata,outFile);
//        out.close();
    }
...
//        BufferedImage originImage = getBufferedImage(path);
        InputStream murl=new URL(path).openStream();
        BufferedImage originImage=ImageIO.read(murl);
        if (originImage != null) {
...
//        String imgName = imageFileNames.get(r);
        int r=(int) Math.round(Math.random() * 4)+1;
        String imgName="vc"+r+"_min.png";
        String path = "http://47.97.67.235/www/template/" + imgName;
